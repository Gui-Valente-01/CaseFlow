...
export function CaseForm({ mode, initial, clients }: Props) {
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <F label="Título do processo" name="title" defaultValue={initial?.title} placeholder="Ex.: Ação de cobrança" required />

      <div className="grid gap-5 sm:grid-cols-2">
        <F label="Número do processo (CNJ)" name="case_number" defaultValue={initial?.case_number} placeholder="0000000-00.0000.0.00.0000" />
        <F label="Tipo" name="type" defaultValue={initial?.type} placeholder="Cível, Trabalhista, etc." />
      </div>
      ...
      <label className="block">
        ...
        <textarea 
          name="next_step"
          defaultValue={initial?.next_step}
          rows={3}
